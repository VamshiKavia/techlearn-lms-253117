...
async def create(payload: CourseCreate, user=Depends(require_roles("instructor", "admin"))):
...
async def update(course_id: str, payload: CourseUpdate, user=Depends(require_roles("instructor", "admin"))):
...
async def delete(course_id: str, user=Depends(require_roles("admin"))):
